...
parser.add_option(
    ...
    dest="width",
    type="int",
    help="how many instances per WIDTH",
    metavar="WIDTH",
    default=3
)
parser.add_option(
    ...
    dest="height",
    type="int",
    help="how many instances per HEIGHT",
    metavar="HEIGHT",
    default=3
)
parser.add_option(
...
roles = (options.roles and set(options.roles.split(',')) or set(get_roles())) - set(options.exclude.split(","))
roles_per_run = len(roles) / (options.width * options.height)
print roles_per_run
...
#     def __init__(self, parent, runners):
#         self.runners_panel = []
...
#                 runner.role
#             )
#             self.runners_panel.append(frame)
#             frame.grid(
#                 row=line_counter,
...
#             frame = [InternalRunner(role) for role in instance_group]
#             self.pages.append(frame)
#         self.runners = InstancesFrame(self.middle_frame, self.pages[0])
#         self.runners.grid(sticky='nwes')
#         self.resizable(0, 0)
...
#         self.runners.update_runners(self.pages[self.current_page])
#         self.status_label['text'] = self.get_status_label()
